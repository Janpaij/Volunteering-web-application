...
<?php
	...
	$q = mysqli_query($con, "SELECT * FROM projects WHERE projectNumber = '4'");
	$row = mysqli_fetch_assoc($q);
	if(ISSET($_POST['signUp'])){
		mysqli_query($con,"UPDATE logins SET signUp = '4' WHERE username = '".$_SESSION['username']."'");
	...
	if(ISSET($_POST['deleteProject'])){
		mysqli_query($con,"DELETE FROM projects WHERE projectNumber = '4'");
		$number = 1;	
		...
		<div id="about">
			...
			if($_SESSION['logged in']){
				...
				if($roww['usertype'] == "organisation"){
					$w = mysqli_query($con, "SELECT * FROM logins WHERE signUp = '4' ");
